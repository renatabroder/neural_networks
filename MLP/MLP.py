...
def mlp(dataset, n_camadas=3, n_perceptron=[5, 5], taxa_erro=0.01, eta=0.5, m=0.5, max_epocas=500):
    ...
    treino, teste, rotulos_treino, rotulos_teste = train_test_split(entradas, saida_esperada, test_size=0.2, random_state=50)
    ...
    #while(epoca < max_epocas):
        ...
        for i in range(len(treino)):
            ...
            if(erro_quadr_padrao > taxa_erro):
                ...
                # atualiza todos os pesos se o erro da camada de saida nao for aceitavel
                for c in range(n_camadas-1, -1, -1):
                    for n in range(n_perceptron[c]):
                        if(c == 0):
                            for p in range(len(pesos[c][n])):
                                pesos[c][n][p] = pesos[c][n][p] + (eta*treino[i][p]*deltas[c][n])

                        else:
                            for p in range(len(pesos[c][n])):
                                pesos[c][n][p] = pesos[c][n][p] + (eta*saida_ativacao[c-1][p]*deltas[c][n])
        ...
        print('Erro de teste: ', round(np.mean([e**2 for e in aux_erros]), 5))
...
def main():
    ...
    dataset = leDados("iris.csv")
    mlp(dataset, n_camadas=2, n_perceptron=[5])
